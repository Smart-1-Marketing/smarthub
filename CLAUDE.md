...
That section opens by saying neither CMS has a write API we can use. It is
true of Smart 1 Sites and it has **not** been true of WordPress since 5.6:
core has had `/wp-json/wp/v2/` since 4.7 and **Application Passwords** since
5.6, so a client's blog posts and image alt text can be written directly with
nothing installed on their site. `hub/wordpress.py` writes them and
`hub/cms_credentials.py` holds what the calls are made with. The Claude path
is not replaced — it still covers Smart 1 Sites, which genuinely has no
content API, and it covers the two kinds this refuses.
