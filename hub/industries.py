"""Industry templates — channels, demand triggers, intro copy and price tiers.

Nine industries, each with the channels we actually sell into it, the demand
triggers that make its media worth timing, and an intro paragraph. Originally
`modules/proposal_builder/industries.py`, moved here when the two proposal
builders were consolidated: the surviving builder needs this library, and a
shared table of what we sell to whom does not belong inside one module.

The price tiers here are a **fallback**. `hub.rate_card.tiers_for()` builds
tiers from real products at real card rates and is preferred everywhere; these
exist for the case where the card cannot be read or an industry's channels map
to nothing on it, because a proposal with no packages is worse than one with
generic ones. See the note in the Proposal Builder's AI section generator.
"""
