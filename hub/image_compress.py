"""Target-size image compression, shared.

Lifted out of modules/image_optimizer so any module can reach it. This is not
the same operation as hub.images.optimise: that caps the longest edge and
converts once, while this iterates — dropping quality, then dimensions, then
palette size — until the file fits a byte budget, shelling out to pngquant and
gifsicle where they are installed.

Both are image processing and both belong in hub/. Collapsing this into
optimise() would have deleted the Image Optimizer's whole reason to exist, so
it moved rather than merged. Moved verbatim: the algorithm is unchanged, only
its address is.
"""
