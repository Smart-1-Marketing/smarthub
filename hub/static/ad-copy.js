 ...
 * media partner out of the record on the screen behind it. Every one of those
 * is on the client's own insertion orders and the seller and the confirmation
 * address are on the signed-in account, so the form now opens on what the Hub
 * already knows and asks only for what nobody else can answer.
 *
 * The controls come from hub/static/knack-form.js — the same drawer the web
 * ticket form uses — because a second copy of "what control does a Knack
 * dropdown need" is a second copy to keep in step. What is here is only what
 * an ad copy request means: the campaign picker that carries its order number
 * and media partner with it, and the client's own URLs offered on the URL
 * question.
 *
 * The prefill is decided server-side, in hub/ad_copy.py, and this draws what
 * comes back. There is deliberately no JavaScript mirror of the rules: target
 * areas and the creative classifier each carry one and each needs a test
 * proving the halves still agree, and that cost is paid twice already.
 *
 * Two entry points, matching /campaign-request.js:
 *
 *   AdCopyRequest.open({client, user, onsaved})   — a caller with the client
 *   AdCopyRequest.pick({user, onsaved})           — look the client up first
 */
window.AdCopyRequest = (function () {
  ...
  // ----------------------------------------------------------- the controls
  // Only the three questions that mean something particular here. Everything
  // else is whatever Knack says the field is.
  function custom(opts) {
    return function (f, v) {
      var o = opts.options || {};
      if (f.key === 'campaign') {
        // The client's own campaigns, newest insertion order first, and still
        // a text box: a campaign being set up today is not on an IO yet, and
        // a picker that refuses it sends the rep to another tool.
        return kf().suggest(PREFIX, f, v, o.campaigns || [],
          'From this client’s insertion orders — or type another.',
          'Which campaign?');
      }
      if (f.key === 'order_number') {
        return kf().suggest(PREFIX, f, v, o.order_numbers || [],
          'Filled in from the campaign above where the IO says which.',
          'Current order number');
      }
      if (f.key === 'media_partner' && f.control !== 'connection' &&
          !(f.choices && f.choices.length)) {
        return kf().suggest(PREFIX, f, v, o.partners || [],
          'From this client’s insertion orders.', 'Media partner');
      }
      if (f.key === 'url_changing' && f.control !== 'boolean' &&
          !(f.choices && f.choices.length)) {
        // Knack publishes no choices for it, so it is a free-text answer —
        // and the answer is nearly always one of the client's own URLs.
        return kf().suggest(PREFIX, f, v, o.urls || [],
          'This client’s websites, from their record.', 'Yes / no — and the new URL');
      }
      if ((f.key === 'url_changing' || f.key === 'change_for') &&
          (f.control === 'boolean' || (f.choices || []).length === 2)) {
        return kf().radios(PREFIX, f, v);
      }
      return null;
    };
  }
  ...
  // insertion order that campaign is on — and only when that IO says which,
  ...
  // what they typed, which is the trap the Smart 1 Ads target-area rows had.
  function link(options) {
    var camp = document.getElementById(PREFIX + 'campaign');
    if (!camp) return;
    camp.addEventListener('change', function () {
      ...
      [['order_number', 'io'], ['media_partner', 'partner']].forEach(function (pair) {
        ...
        var meta = document.querySelector('[data-meta="' + PREFIX + pair[0] + '"]');
        if (vals.length === 1) {
          el.value = vals[0];
          if (meta) meta.innerHTML = '<div class="muted" style="font-size:11.5px;margin-top:3px">' +
            'From the insertion order for this campaign.</div>';
        } else if (meta) {
          // Two orders on one campaign is a real answer and not one this can
          // pick between — say so rather than filling in the first.
          meta.innerHTML = vals.length > 1
            ? '<div style="font-size:11.5px;margin-top:3px;color:#b45309">This campaign is on ' +
              vals.length + ' insertion orders — pick one.</div>'
            : '';
        }
      });
    });
  ...
  function open(opts) {
    ...
    var m = K.shell('adCopyModal', 'Ad Copy Request — ' + esc(name),
      '<div class="empty">Reading the ad copy fields from Knack… <span class="spin"></span></div>', 660);
    ...
    fetch('/api/client/ad-copy/fields?client=' + encodeURIComponent(name))
      ...
      .then(function (d) {
        ...
        var ctx = { options: options, custom: custom({ options: options }) };

        body.innerHTML = K.notesHtml(d.notes) +
          K.form(PREFIX, fields, d.values || {}, null, ctx);
        K.wire(body);
        link(options);
        ...
        document.getElementById('acSend').onclick = function () {
          ...
          fetch('/api/client/ad-copy', {
            ...
            if (r.rejected && r.rejected.length) {
              ...
              // missing instead of finding out from the campaign team later.
              body.innerHTML = K.refusedHtml(r.rejected) +
  ...
  // screen. It is a search of real clients and never a text box, for the reason
  ...
  function pick(opts) {
    ...
    var m = K.shell('adCopyPickModal', esc(opts.title || 'Ad Copy Request'),
      ...
      '<div id="acPickList" style="margin-top:10px;max-height:46vh;overflow-y:auto"></div>', 520);
