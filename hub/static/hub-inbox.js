(function () {
  ...
  css.rel = 'stylesheet'; css.href = '/assets/help-center.css?v=qa-inbox-1'; document.head.appendChild(css);
  ...
  function paint() {
    ...
    items.forEach(item => {
      ...
      if (item.unread !== false && seen[item.id] !== (item.revision || status)) count++;
    ...
    if (!items.length) panel.querySelector('.hub-inbox-list').textContent = 'No recent notifications.';
  ...
  function mount() {
    ...
    control.querySelector('[data-read]').onclick = () => { data.items.forEach(i => { seen[i.id] = i.revision || states[i.id] || i.status; }); save(); paint(); };
  ...
  async function refresh() {
    ...
    try {
      ...
      panel.querySelector('[data-inbox-error]').textContent = data.qa_error || '';
      paint();
