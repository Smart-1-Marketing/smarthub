...
every table here is ``hub/client_key.py``'s Hub-wide key, never a plain
display name -- a mapping a rep files by hand through
``/reports/unmapped`` resolves through ``store.resolve_client()`` first, so
a run's own rows have to resolve through the identical function or they
land under a different ``client`` string than the campaigns a rep maps for
the same business, and the two halves of one client's reporting never see
each other.
