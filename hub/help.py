...
REGISTRY: list[Help] = [
    ...
    _h("seo.wordpress.workflow", "Send blog drafts, alt text and schema to WordPress",
       ...
       link="/seo", link_text="Open SEO Clients", ask_only=True),
    ...
    _h("hub.client360.header", "Client 360 and its next action",
       "Client 360 opens a compact Overview; use the section rail and its badges for more detail. The next-action line highlights the highest-priority finding from existing cards. Open its destination and review the evidence before acting. Nothing urgent on file describes available readings; unmeasured data is labeled separately. A card's Refresh retries just that card. Pipeline & leads reads the client's own Suite sub-account. Client Assets opens Client Uploads, Creative, Hub Projects, Logos and Internal.", step=1,
    ...
    _h("seo.schema.date_added", "Mark it once it's live",
       "Date added records deployment and is editable. For WordPress, sending schema first checks that the Smart 1 Hub plugin accepts the saved block. A stored value is not proof the visitor's page prints it: use Is it live? to check the public page. Review missing or different markup before treating deployment as complete. A date alone does not establish that the current schema is visible."),
    ...
    _h("image_picker.gallery.saved", "One client asset home",
       'The client asset home always offers Client Uploads, Creative, Hub Projects, Logos and Internal, including empty folders. Open project continues work in its original tool; listing a render does not approve it. Choose a folder when uploading. Original images are kept while web-ready SEO copies are prepared in the background. Details shows source and Suite delivery status separately: saved in the Hub does not mean delivered to Suite. A source warning means some records could not be read. Unassigned files need review before appearing under a client.'),
    ...
    _h('google_access.cleanup.delete', 'Check a GTM tag on a site before cleanup',
       'GA4 inactivity uses events and sessions in the displayed window. GTM has no traffic-reporting API: absent confirmed linked GA4 activity initially makes a container a candidate. The scan then checks candidate sites it can resolve for the container ID. A tag found on a page moves the row to Needs Review; that proves installation, not traffic. If no site was resolved, use Check site to supply or correct its URL. Both automatic and manual checks search raw HTML without executing the page. Not found on one page is not proof of no use elsewhere. Review the account, resource and evidence before deleting. GA4 goes to Analytics trash; GTM deletion is permanent.',
       link='/tools/google-access/qa-inactive/', link_text="Open tool"),
    ...
    _h('proposal_execution.overview.autostart', 'Won proposals start their own plan',
       'When a quote in the Proposal Builder is marked Approved (the client accepted it at their link) or Converted (an insertion order was written), a plan is started for it within the hour without anybody pressing Analyze -- the same plan the button builds, with a note on the run and on the quote\'s activity saying it was automatic. Nothing is kept or answered by that: the plan arrives as proposals for the client\'s owner to review, and it shows on Client 360 and on My Clients like any other. A client who already has a different run open is named rather than superseded, because superseding carries approved work forward and that is a person\'s decision. This button runs the same sweep now instead of waiting for the hour.',
       link='/proposal-execution', link_text="Open tool"),
    ...
    _h('proposal_execution.plan.questions', 'Answer what the proposal does not say',
       'The analysis asks rather than guesses: a channel with no budget beside it, a campaign with no start date, creative whose supplier the proposal never names. Each question says why it is being asked. Where the proposal or the quote does answer, the answer is filled in from the document and marked as such, and you can change it. The answers are read by the work: the launch date sets the due date on every task, the supplier marks each creative item, and the working briefs and handoff packets carry them. A client who answers on their own page appears beside the question with what they said and who said it; Use their answer takes it onto the plan, and until somebody does it stays a proposal and is counted as one to confirm. The shared inputs above them are the facts every background task needs once.',
       link='/proposal-execution', link_text="Open tool"),
    ...
    _h('proposal_execution.plan.client_link', 'The client\'s own page',
       'Create client link makes a page the client can open without a login, at an address nobody can guess, listing only what we need from them: the files they agreed to supply with the date each is wanted by, their upload link, the questions that are theirs to answer, and who at Smart 1 to talk to. They can answer those questions on the page and hand a file back to Smart 1 to produce; what they send lands beside the question here as a proposal with their name on it, never straight onto the plan. Nothing internal reaches it. Copy the link and send it; it updates as the plan changes. Revoke stops the address working, and a new link can be made afterwards.',
       link='/proposal-execution', link_text="Open tool"),
    _h('proposal_execution.plan.progress', 'Done, landed and overdue',
       'Once the launch date is answered every kept launch task and creative item has a date, and the plan says where each stands. Done is a press somebody made, with their name and the day on it. Landed is what the Hub saw for itself: creative filed against this client since the plan was made -- a display pack attached, a commercial approved, a mix filed, or the client uploading through their link -- named with the tool and the day, and never written down. Overdue is a kept item past its date with neither. An overdue item reaches its owner\'s My Clients list, and leaves it the moment it is marked done or the file arrives. A log the Hub could not read is said on the item rather than read as nothing having landed.',
       link='/proposal-execution', link_text="Open tool"),
