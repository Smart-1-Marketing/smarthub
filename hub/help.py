...
REGISTRY: list[Help] = [
    ...
    _h('proposal_execution.plan.review', 'Check the plan the proposal produced',
       'Three lists come out of the analysis: the creative that has to exist, what happens once before launch, and what the proposal promises every month. Each item is a proposal until you tick it to keep it or mark it not needed; an item found by AI quotes the line it came from, and one with no matching line in the proposal is flagged so you can check it before keeping it. Add anything the analysis missed with the box under each list. Only what you keep reaches the working briefs and handoff packets.',
       link='/proposal-execution', link_text="Open tool"),
    _h('proposal_execution.plan.questions', 'Answer what the proposal does not say',
       'The analysis asks rather than guesses: a channel with no budget beside it, a campaign with no start date, creative whose supplier the proposal never names. Each question says why it is being asked. Where the proposal does answer, the answer is filled in from the document and marked as such, and you can change it. The shared inputs above them are the facts every background task needs once.',
       link='/proposal-execution', link_text="Open tool"),
