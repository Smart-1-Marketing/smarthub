...
export function clientProofHtml(proof:ClientProof) {
  ...
  // Cells are served from disk instead of inlined as base64: a Meta set with
  // the story and the square at 2x was ten megabytes of HTML per open on a
  // phone. Now the page is a few kilobytes and the images stream separately,
  // cached by the browser between opens.
  const cells=proof.cells.map((c,i)=>{const key=cellKey(c);return `<article data-cell="${esc(key)}"><h2>${esc(c.size)} · ${esc(c.platform)}</h2><img alt="${esc(c.conceptId+' '+c.size)}" src="/client-proof/${esc(proof.token)}/cell/${i}" loading="lazy">${noteList(key)}${noteBox(key,'this ad')}</article>`;}).join('');
