...
Read-only GA4 diagnostic. It uses the existing Google account index and OAuth
connection, pulls the sessions GA4 currently places in Unassigned, and turns
those rows into a remediation queue. Historical attribution is never rewritten;
the fixes are recommendations for future traffic.
"""
...
RECOGNIZED_MEDIA = {
    ...
    "paid_social", "referral", "social", "video", "(none)",
}
PAID_HINTS = {"cpc", "ppc", "paid", "paidsearch", "paid_search", "sem", "display", "cpm", "cpv"}
PAYMENT_HOSTS = ("paypal.", "stripe.", "squareup.", "authorize.net", "afterpay.", "klarna.")
...
def diagnose_row(row: dict, client_domain: str = "") -> dict:
    """Classify one GA4 Unassigned row without inventing a cause.

    The highest-confidence finding wins. `confidence` is intentionally exposed
    because a source/medium row can suggest a configuration issue but cannot
    prove what happened before the session reached GA4.
    """
    ...
    if source in EMPTY and medium in EMPTY:
        ...
        fix = "Review cross-domain measurement, referral exclusions, checkout/subdomain transitions, and tag coverage."
        confidence = "high"
    elif any(h in source for h in PAYMENT_HOSTS):
        kind = "payment_referral"
        title = "A payment provider is taking credit for the session"
        why = "Visitors are returning from a checkout/payment domain and GA4 is treating that return as a new referral."
        ...
        fix = "Verify ad-platform auto-tagging and add utm_campaign to any manually tagged paid links."
        confidence = "medium"
        ...
        why = "The values look like paid Google traffic, so this may be inconsistent tagging or an Ads/GA4 integration issue."
        fix = "Verify Google Ads auto-tagging, the GA4↔Google Ads link, redirects preserving gclid/UTMs, and consistent medium values."
        confidence = "medium"
...
def _suggest_utm(source: str, medium: str, campaign: str) -> str:
    ...
    med = medium if medium in RECOGNIZED_MEDIA and medium != "(none)" else "cpc" if medium in PAID_HINTS else "referral"
    camp = re.sub(r"[^a-zA-Z0-9_-]+", "-", campaign or "campaign-name").strip("-") or "campaign-name"
    return f"utm_source={src}&utm_medium={med}&utm_campaign={camp}"


def summarize(rows: list[dict], total_sessions: int) -> dict:
    diagnosed = [diagnose_row(r, r.get("client_domain", "")) for r in rows]
    unassigned = sum(r["sessions"] for r in diagnosed)
    issues = Counter()
    ...
    return {
        ...
        "unassigned_rate": round((unassigned / total_sessions * 100), 2) if total_sessions else 0,
        "rows": diagnosed,
...
def _property_for_client(client: str, domain: str = "") -> dict:
    ...
    prop = rid.split("/")[-1]
    return {"ok": True, "property_id": prop, "property_name": item.get("name") or prop,
            "google_login": item.get("google_login") or "", "index": found}
...
def live_analysis(client: str, domain: str = "", days: int = 30) -> dict:
    ...
    detail = _run_report(token, mapped["property_id"], {
        ...
        "dimensions": [
            ...
            {"name": "sessionCampaignName"}, {"name": "landingPagePlusQueryString"},
        ],
        "metrics": [{"name": "sessions"}],
    ...
    out = summarize(rows, _metric(total))
    out.update({"ok": True, "client": client, "days": days,
                "property_id": mapped["property_id"], "property_name": mapped["property_name"],
                "google_login": mapped["google_login"], "index_stale": mapped["index"].get("stale", False)})
    return out
...
def _available_clients() -> list[dict]:
    ...
    for item in google_index.load().get("items") or []:
        ...
        if name and name.lower() not in seen:
            seen[name.lower()] = {"name": name, "property": item.get("name") or item.get("resource_id") or ""}
...
<div class="card section"><h2>What is causing it</h2><div class="issues" id="issues"></div></div>
