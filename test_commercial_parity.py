"""The Commercial Builder half of the builder-parity list.

Two gaps from that list, both of which were missing in the Commercial Builder
and present in Radio Promo — so both are ported in the direction they were
missing:

* **One reader for "the copy has to actually say it" (§2.2).** Radio Promo
  checked its scripts literally, refusing the write rather than trusting the
  prompt. The Commercial Builder asked whether a field was *set on a client
  record*, which is a different and much easier question: the renderer draws
  the end card from `project.cta` and only where a CTA scene exists, so a spot
  whose end card had been deleted passed with the phone number sitting in a
  database column. `hub/script_contents.py` is the shared rule, and the three
  false positives it closes are asserted beside the findings it must keep
  making.
* **The finished spot reaches Smart 1 Suite (§2.7)**, through the Hub's one
  contact write path rather than a second raw webhook, revising the
  opportunity it already opened rather than adding a second to the same
  client's pipeline.

The Radio Promo half of that list is deliberately not here. PR #336 landed the
same slots-and-QC work on main from another direction while this was in
flight, so those changes are being reworked onto its structure rather than
merged over the top of it — two implementations of one feature is the failure
`CLAUDE.md` spends a section on, and the resolution belongs in its own change.
...
for _k in ("OPENAI_API_KEY", "CLOUDINARY_URL", "GHL_PRIVATE_TOKEN"):
